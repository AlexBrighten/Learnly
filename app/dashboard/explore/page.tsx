...
import {
  Search, BookOpen, Clock, TrendingUp, ExternalLink,
...
function SearchResultCard({ result, onCreateCourse, onPreview, index }: { result: any; onCreateCourse: (topic: string) => void; onPreview: (result: any) => void; index: number }) {
  return (
    <motion.div
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <div className="flex items-center flex-wrap gap-3 mt-4 pt-3 border-t border-border/50">
          ...
          <button
            ...
            className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-muted text-muted-foreground hover:text-foreground text-sm font-bold shadow-sm hover:bg-muted/80 hover:-translate-y-0.5 active:translate-y-0 transition-all focus:ring-2 focus:ring-offset-2 focus:ring-primary focus:outline-none"
          >
...
function TopicSection({
  ...
  loading: boolean;
  tintClass: string;
  onCreateCourse: (topic: string) => void;
    ...
    <section className="space-y-4">
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            <div className={`p-2 rounded-xl bg-gradient-to-br ${tintClass} border border-white/10 text-foreground`}>
              <Icon className="w-6 h-6" />
            </div>
            <h3 className="text-2xl font-black text-foreground">{title}</h3>
          </div>
          <p className="text-muted-foreground text-sm font-medium pl-12 max-w-xl">{description}</p>
        </div>
        <div className="hidden sm:inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-card border border-border text-xs font-bold text-muted-foreground">
          <Clock className="w-3.5 h-3.5" />
          Live Wikipedia results
        </div>
      </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 gap-5">
          {[1, 2].map((item) => (
            <div key={item} className="flex gap-4 p-5 rounded-2xl bg-card/60 border border-border animate-pulse overflow-hidden">
        ...
        <div className="grid grid-cols-1 gap-5">
          {items.slice(0, 4).map((item, index) => (
            <SearchResultCard key={`${title}-${index}`} index={index} result={item} onCreateCourse={onCreateCourse} onPreview={onPreview} />
          ))}
        </div>
      ) : (
        <div className="p-6 rounded-2xl border border-dashed border-border/60 bg-card/50 text-center text-muted-foreground">
          Wikipedia did not return live results for this section right now.
        </div>
      )}
    </section>
  );
...
function ExploreContent() {
  ...
  const [scienceTopics, setScienceTopics] = useState<any[]>([]);
  const [technologyTopics, setTechnologyTopics] = useState<any[]>([]);
  ...
  useEffect(() => {
    if (previewItem) {
      setFullExtract(null);
      setLoadingFull(true);
      fetch(`/api/wiki-full?title=${encodeURIComponent(previewItem.title)}`)
        .then(res => res.json())
        .then(data => {
            if (data.extract) setFullExtract(data.extract);
        })
        .finally(() => setLoadingFull(false));
    }
  }, [previewItem]);

  const fetchWikipediaTopics = useCallback(async (topic: string) => {
    const response = await fetch(`/api/explore-search?q=${encodeURIComponent(topic)}`);
  ...
  useEffect(() => {
    ...
    const loadTopics = async () => {
      ...
      try {
        const [science, technology] = await Promise.all([
          fetchWikipediaTopics("science"),
          fetchWikipediaTopics("technology"),
        ]);
        ...
        setScienceTopics(science);
        setTechnologyTopics(technology);
      } catch {
        if (isActive) {
          setScienceTopics([]);
          setTechnologyTopics([]);
  ...
  return (
    <div className="min-h-screen pb-20 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 xl:px-12 space-y-10 pt-4">
            ...
            >
              ...
              {loading ? (
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
                  {results.map((r, i) => (
                    <SearchResultCard key={i} index={i} result={r} onCreateCourse={handleCreateCourse} onPreview={setPreviewItem} />
                  ))}
                </div>
              ) : (
                <motion.div
        ...
        {/* Categories & Live Wikipedia Topics — shown when not searching */}
        <AnimatePresence>
          {!searched && (
            <motion.div
              initial={{ opacity: 0 }}
            ...
            >
              ...
              <div className="space-y-6">
                <div className="flex items-end justify-between">
                  <div className="space-y-1">
                    <div className="flex items-center gap-3">
                      <div className="p-2 rounded-xl bg-gradient-to-br from-emerald-500/20 to-cyan-500/20 text-emerald-500 border border-white/10">
                        <TrendingUp className="w-6 h-6" />
                      </div>
                      <h2 className="text-2xl font-black text-foreground">Latest Wikipedia Topics</h2>
                    </div>
                    <p className="text-muted-foreground text-sm font-medium pl-12 hidden sm:block">Live science and technology topics pulled directly from Wikipedia search results.</p>
                  </div>
                  <div className="hidden lg:inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-card border border-border text-xs font-bold text-muted-foreground">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                  <TopicSection
                    title="Science"
                    description="Fresh science topics, discoveries, and concepts surfaced from live Wikipedia results."
                    icon={Lightbulb}
                    items={scienceTopics}
                    loading={topicsLoading}
                    tintClass="from-cyan-500/20 to-blue-500/20 text-cyan-500"
                  ...
                  <TopicSection
                    title="Technology"
                    description="Emerging technology topics, platforms, and breakthroughs from Wikipedia."
                    icon={Zap}
                    items={technologyTopics}
                    loading={topicsLoading}
                    tintClass="from-indigo-500/20 to-violet-500/20 text-indigo-500"
                    onCreateCourse={handleCreateCourse}
                    onPreview={setPreviewItem}
                  />
                </div>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
              ...
              >
                ...
                {previewItem.thumbnail ? (
                  ...
                  <div className="w-full h-32 sm:h-40 bg-gradient-to-br from-primary/10 to-purple-500/10 flex items-center justify-center shrink-0">
                ...
                <div className="p-6 sm:p-8 flex-1 overflow-y-auto">
                  <div className="flex items-start justify-between gap-4 mb-4">
                    <div>
                      <h2 className="text-2xl sm:text-3xl font-black text-foreground mb-1">{previewItem.title}</h2>
                  ...
                  <div className="prose prose-sm sm:prose-base dark:prose-invert max-w-none text-muted-foreground leading-relaxed">
                    {loadingFull ? (
                       <div className="flex flex-col items-center justify-center py-10 space-y-4">
                           ...
                           <p className="text-sm font-medium animate-pulse text-muted-foreground">Gathering comprehensive knowledge from Wikipedia...</p>
                       </div>
                    ) : fullExtract ? (
                        fullExtract.split('\n\n').filter(p => p.trim() !== '').map((para, i) => {
                            if (para.startsWith('==')) {
                                return <h3 key={i} className="text-xl font-bold text-foreground mt-8 mb-3">{para.replace(/=/g, '').trim()}</h3>;
                            }
                            return <p key={i} className="mb-4">{para}</p>;
                        })
                    ) : (
                        <p>{previewItem.extract || "No description available for this topic."}</p>
                    )}
                  </div>
                </div>

                <div className="p-6 sm:p-8 pt-4 border-t border-border/50 shrink-0 bg-card flex items-center justify-end gap-3 flex-wrap">
                  <a
                    href={previewItem.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="px-5 py-3 rounded-xl hover:bg-muted text-muted-foreground hover:text-foreground text-sm font-bold transition-all flex items-center gap-2"
                  >
                  ...
                  <button
                    onClick={() => {
                        handleCreateCourse(previewItem.topic);
                    }}
                    className="px-6 py-3 rounded-xl bg-primary text-primary-foreground text-sm font-bold shadow-lg shadow-primary/20 hover:shadow-primary/40 hover:-translate-y-0.5 active:translate-y-0 transition-all focus:ring-2 focus:ring-offset-2 focus:ring-primary focus:outline-none flex items-center gap-2"
                  >
                    <Sparkles className="w-4 h-4" />
                    Generate Course
                  </button>
