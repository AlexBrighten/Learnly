...
export default function CourseDetailPage() {
    ...
    if (!course) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
                ...
                <p className="text-slate-400 mb-6">This course may have been deleted.</p>
    ...
    const getChapterStatus = (idx) => {
        ...
        // First chapter is always unlocked. Next unlocked = first incomplete
    ...
    return (
        <div className="max-w-4xl mx-auto">
            ...
            <motion.div
                ...
                className="rounded-3xl bg-gradient-to-br from-indigo-600/20 via-purple-700/20 to-indigo-900/20 border border-white/10 p-8 mb-10"
            >
                <div className="flex flex-col sm:flex-row items-start justify-between gap-6">
                    <div className="space-y-3">
                        <h1 className="text-3xl font-bold text-white">{course.title}</h1>
                        <p className="text-slate-400 max-w-xl">{course.summary}</p>
                                ...
                                >
                                    ...
                                    <div>
                                        <div className="flex items-center gap-3 mb-1">
                                            <span className="text-xs text-slate-500 font-medium">CHAPTER {idx + 1}</span>
                                            {status === "completed" && (
                                        ...
                                        <h3 className={`text-lg font-semibold transition-colors ${
                                            status === "locked" ? "text-slate-500" : "text-white group-hover:text-indigo-300"
                                        }`}>
                                            {chapter.title}
                                        </h3>
                                        <p className="text-sm text-slate-500 mt-1">{chapter.summary}</p>
                                    </div>
                                </button>
                                            ...
                                            return (
                                                ...
                                                >
                                                    <motion.div
                                                        ...
                                                        whileTap={{ scale: 0.95 }}
                                                        className={`p-4 rounded-xl bg-gradient-to-br ${matInfo.color} bg-opacity-10 border border-white/10 hover:border-white/20 transition-all text-center`}
                                                    >
                                ...
                                {isLast && (
                                    <div className="relative pl-20 pb-6">
                                        <div className={`absolute left-4 top-0 w-9 h-9 rounded-full flex items-center justify-center z-10 ${
                                            ...
                                            <Trophy className={`w-5 h-5 ${progress === 100 ? "text-white" : "text-slate-600"}`} />
                                        </div>
                                        <p className={`text-sm font-medium pt-2 ${
                                            progress === 100 ? "text-yellow-400" : "text-slate-600"
                                        }`}>
